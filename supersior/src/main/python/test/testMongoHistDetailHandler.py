...
class TestMongoHistDetailHandler(unittest.TestCase):
    def setUp(self):
        self.handler = MongoHistDetailHandler()

    def test_insert_histdetail(self):
        histdetail = SHistDetail.extract(params.s_hist_data_file)
        str_id = self.handler.get_id(histdetail.stock_code, histdetail.date)
        json_histdetail = MongoHistDetailHandler.get_json(histdetail)
        self.handler.table.remove({'_id': str_id})
        self.handler.insert(json_histdetail)
        ret = self.handler.query_by_date(histdetail.stock_code, histdetail.date)
        MongoHistDetailHandler.decompress(json_histdetail)
    ...
    def test_compress(self):
        ...
        """
        hist_detail = SHistDetail.extract(params.s_hist_data_file)
